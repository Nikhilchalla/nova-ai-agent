...
const AGENT_TOOLS = [
  { id: "web_search", icon: "🔍", label: "Web Search", color: "#ffffff" },
  { id: "summarize", icon: "📋", label: "Summarize", color: "#ffffff" },
  { id: "analyze", icon: "🧠", label: "Analyze", color: "#ffffff" },
  { id: "code", icon: "💻", label: "Write Code", color: "#ffffff" },
  { id: "plan", icon: "🗺️", label: "Plan & Strategy", color: "#ffffff" },
];
...
function MessageBubble({ msg }) {
  ...
  if (isSystem) {
    return (
      <div style={{
        textAlign: "center", padding: "6px 16px", margin: "8px auto",
        background: "rgba(255,255,255,0.04)", borderRadius: 20, fontSize: 12,
        color: "#555555", maxWidth: 400, border: "1px solid rgba(255,255,255,0.08)"
      }}>
        {msg.content}
      </div>
    );
    ...
    <div style={{
      ...
      <div style={{
        ...
        background: isUser
          ...
          : "rgba(20,20,20,0.9)",
        border: isUser ? "1px solid rgba(255,255,255,0.1)" : "1px solid rgba(255,255,255,0.06)",
        borderRadius: isUser ? "20px 20px 4px 20px" : "4px 20px 20px 20px",
        padding: "10px 14px",
        backdropFilter: "blur(10px)",
        ...
        {msg.toolUsed && (
          <div style={{
            fontSize: 11, color: "#aaaaaa", marginBottom: 8,
            display: "flex", alignItems: "center", gap: 4, opacity: 0.8
          }}>
            <span>⚡</span> Used {msg.toolUsed}
          </div>
        ...
        <div style={{
          color: isUser ? "#e2e8f0" : "#cccccc", fontSize: 14,
          lineHeight: 1.7, whiteSpace: "pre-wrap"
        }}
          dangerouslySetInnerHTML={{ __html: formatMessage(msg.content) }}
        />
...
function formatMessage(text) {
  return text
    .replace(/```(\w+)?\n?([\s\S]*?)```/g, '<pre style="background:#111111;padding:12px;border-radius:8px;overflow-x:auto;font-family:monospace;font-size:13px;border:1px solid rgba(255,255,255,0.08);margin:8px 0"><code style="color:#cccccc">$2</code></pre>')
    .replace(/`([^`]+)`/g, '<code style="background:#111111;padding:2px 6px;border-radius:4px;font-family:monospace;font-size:13px;color:#ffffff">$1</code>')
    .replace(/\*\*([^*]+)\*\*/g, '<strong style="color:#ffffff">$1</strong>')
    .replace(/^### (.+)$/gm, '<h3 style="color:#aaaaaa;margin:12px 0 6px;font-size:15px">$1</h3>')
    .replace(/^## (.+)$/gm, '<h2 style="color:#ffffff;margin:12px 0 6px;font-size:16px">$1</h2>')
    .replace(/^- (.+)$/gm, '<div style="display:flex;gap:8px;margin:4px 0"><span style="color:#ffffff;margin-top:2px">▸</span><span>$1</span></div>')
    .replace(/^\d+\. (.+)$/gm, '<div style="display:flex;gap:8px;margin:4px 0"><span style="color:#aaaaaa">●</span><span>$1</span></div>')
    .replace(/\n/g, '<br/>');
}
    ...
    <div style={{
      ...
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @keyframes slideIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
        @keyframes pulse { 0%, 80%, 100% { transform: scale(0.7); opacity: 0.4; } 40% { transform: scale(1); opacity: 1; } }
        @keyframes glow { 0%, 100% { box-shadow: 0 0 20px rgba(255,255,255,0.05); } 50% { box-shadow: 0 0 30px rgba(255,255,255,0.15); } }
        @keyframes spin { to { transform: rotate(360deg); } }
        textarea:focus { outline: none; }
        textarea { resize: none; }
        button:hover { opacity: 0.85; transform: scale(0.98); }
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            width: 42, height: 42, borderRadius: "50%",
            background: "linear-gradient(135deg, #333333, #666666, #333333)",
            display: "flex", alignItems: "center", justifyContent: "center",
            fontSize: 20, animation: "glow 3s ease-in-out infinite",
            boxShadow: "0 0 20px rgba(255,255,255,0.08)"
          }}>✦</div>
          <div>
            <div style={{ fontFamily: "'Space Mono', monospace", fontWeight: 700, fontSize: 18, letterSpacing: 2, color: "#ffffff" }}>
              NOVA
            </div>
            <div style={{ fontSize: 11, color: "#555555", letterSpacing: 1 }}>
              NEURAL OPTIMIZED VELOCITY AGENT
            </div>
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <div style={{
            ...
            borderRadius: 20, padding: "4px 12px", fontSize: 12, color: "#888888"
          }}>
          ...
          <button onClick={clearChat} style={{
            background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)",
            borderRadius: 8, padding: "6px 14px", color: "#666666", fontSize: 12
          }}>
            New Chat
          </button>
        </div>
      ...
      {messages.length <= 2 && (
        <div style={{ width: "100%", padding: "0 24px 16px" }}>
          <div style={{ fontSize: 11, color: "#333333", marginBottom: 10, letterSpacing: 1 }}>TRY ASKING</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
            {quickPrompts.map(p => (
              <button key={p} onClick={() => setInput(p)} style={{
                background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)",
                borderRadius: 20, padding: "8px 14px", color: "#666666", fontSize: 12,
                fontFamily: "inherit"
              }}>{p}</button>
            ))}
          </div>
      ...
      <div style={{ width: "100%", padding: "0 24px 10px" }}>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {AGENT_TOOLS.map(tool => (
            <button key={tool.id} onClick={() => setSelectedTool(selectedTool === tool.id ? null : tool.id)} style={{
              background: selectedTool === tool.id ? "rgba(255,255,255,0.08)" : "rgba(255,255,255,0.02)",
              border: `1px solid ${selectedTool === tool.id ? "rgba(255,255,255,0.3)" : "rgba(255,255,255,0.06)"}`,
              borderRadius: 20, padding: "5px 12px", fontSize: 12,
              color: selectedTool === tool.id ? "#ffffff" : "#444444",
              display: "flex", alignItems: "center", gap: 5, fontFamily: "inherit"
            }}>
              <span>{tool.icon}</span> {tool.label}
            </button>
      ...
      <div style={{
        ...
        <div style={{ width: "100%" }}>
          <div style={{
            display: "flex", gap: 10, alignItems: "flex-end",
            background: "rgba(15,15,15,0.9)", border: "1px solid",
            borderColor: input ? "rgba(255,255,255,0.2)" : "rgba(255,255,255,0.06)",
            borderRadius: 16, padding: "8px 8px 8px 16px",
            ...
            <button onClick={sendMessage} disabled={!input.trim() || isLoading} style={{
              width: 40, height: 40, borderRadius: 10, border: "none",
              background: input.trim() && !isLoading ? "#ffffff" : "rgba(255,255,255,0.05)",
              color: input.trim() && !isLoading ? "#000000" : "#333333",
              fontSize: 16, display: "flex", alignItems: "center", justifyContent: "center",
              flexShrink: 0,
            }}>
              {isLoading ? (
          ...
          <div style={{ textAlign: "center", fontSize: 11, color: "#222222", marginTop: 8 }}>
            Press Enter to send • Shift+Enter for new line • Select a tool above to guide responses
          </div>
        </div>
      </div>
